...
- **Execution policies** — Issues can carry a review/approval execution policy with multi-stage signoff workflows. Reviewers and approvers are selected per-stage, and ATV-Teams routes the issue through each stage automatically. ([#3222](https://github.com/paperclipai/paperclip/pull/3222))
...
- **Standalone MCP server** — New `@paperclipai/mcp-server` package exposing the ATV-Teams API as an MCP tool server, including approval creation. ([#2435](https://github.com/paperclipai/paperclip/pull/2435))
