...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only ATV-Teams.
...
- ATV-Teams must not require extra top-level fields for skill validity
- ATV-Teams-specific extensions must live under `metadata.paperclip` or `metadata.sources`
...
For ATV-Teams, the preferred fidelity extension is:
...
- issue/task ATV-Teams-only metadata
...
- ATV-Teams tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested ATV-Teams shape:
...
Additional rules for ATV-Teams exporters:
...
- omit empty and default-valued ATV-Teams fields when possible
...
- ATV-Teams exporters should emit `.paperclip.yaml` as a sidecar by default
...
## 19. ATV-Teams Mapping

ATV-Teams can map this spec to its runtime model like this:
...
- ATV-Teams extension:
  - `.paperclip.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other ATV-Teams-specific fidelity

Inline ATV-Teams-only metadata that must live inside a shared markdown file should use:

- `metadata.paperclip`

That keeps the base format broader than ATV-Teams.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only ATV-Teams.

## 20. Cutover

ATV-Teams should cut over to this markdown-first package model as the primary portability format.

`paperclip.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For ATV-Teams, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
