...
How to get OpenClaw running in a Docker container for local development and testing the ATV-Teams OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

ATV-Teams includes an end-to-end join smoke harness:
...
- probes and prints a ATV-Teams host URL that is reachable from inside OpenClaw Docker
...
If your ATV-Teams deployment is `authenticated`, provide auth context:
...
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host ATV-Teams server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed ATV-Teams URL (typically `http://host.docker.internal:3100`).
