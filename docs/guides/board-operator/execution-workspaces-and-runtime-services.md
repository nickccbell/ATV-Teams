...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in ATV-Teams.

ATV-Teams now presents this as a workspace-command model:
...
- ATV-Teams does not automatically start or stop these workspace services as part of issue execution.
- ATV-Teams also does not automatically restart workspace services on server boot.
...
2. ATV-Teams realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. ATV-Teams persists execution-workspace metadata such as paths, refs, and provisioning settings.
