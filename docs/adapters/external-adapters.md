...
| Distribution | Ships with ATV-Teams | Published to npm or linked via `file:` |
| Updates | Requires ATV-Teams release | Independent versioning |
...
export async function execute(
  ...
  // 2. Build environment with ATV-Teams vars injected
...
Local adapters are symlinked into ATV-Teams's adapter directory. Changes to the source are picked up on server restart.
...
Other ATV-Teams users can then install your adapter by package name from the UI or API.
