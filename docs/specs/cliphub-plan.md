# ClipHub: Marketplace for ATV-Teams Team Configurations
...
> The "app store" for whole-company AI teams — pre-built ATV-Teams configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for ATV-Teams-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | ATV-Teams company (agents, projects, governance) |
...
A complete ATV-Teams company configuration:
...
Single-agent configurations designed to plug into a ATV-Teams org:
...
Portable skill files that any ATV-Teams agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with ATV-Teams's skill loading system
...
interface Listing {
  ...
  paperclipVersionMin: string;     // Minimum ATV-Teams version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target ATV-Teams company
...
| `POST` | `/api/listings/:id/install` | Install to ATV-Teams company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target ATV-Teams company (or create new)
  → ClipHub API calls ATV-Teams API to:
  ...
  → Redirect to ATV-Teams dashboard with new team running
```
...
### 5.3 Creator: Export from ATV-Teams → Publish

```
Running ATV-Teams company → "Export as Blueprint" (CLI or UI)
  → ATV-Teams exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (ATV-Teams brand blue/purple)
...
| Install flow | One-click deploy to ATV-Teams company |
...
{
  "targetCompanyId": "uuid",         // Existing ATV-Teams company
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as ATV-Teams
- **Backend**: Node.js API (or extend ATV-Teams server)
- **Database**: Postgres (can share ATV-Teams's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with ATV-Teams auth (or OAuth2)

### 9.2 Integration with ATV-Teams

ClipHub can be:
- **Option A**: A separate app that calls ATV-Teams's API to install blueprints
- **Option B**: A built-in section of the ATV-Teams UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing ATV-Teams UI and API.
...
- [ ] Install flow: blueprint → ATV-Teams company
...
- [ ] "Export from ATV-Teams" CLI command
