...
When you rate an agent's response with **Helpful** (thumbs up) or **Needs work** (thumbs down), ATV-Teams saves your vote locally alongside your running instance. This guide covers what gets stored, how to access it, and how to export it.
...
feedback-export-20260331T120000Z/
  ...
  traces/
    PAP-123-e5f6g7h8.json      # ATV-Teams feedback envelope (one per trace)
  full-traces/
    PAP-123-e5f6g7h8/
...
Exports are full by default. `traces/` keeps the ATV-Teams envelope, while `full-traces/` contains the richer per-trace bundle plus any recoverable adapter-native files.
...
- Default endpoint: when no feedback export backend URL is configured, ATV-Teams falls back to `https://telemetry.paperclip.ing`
