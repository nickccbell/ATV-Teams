...
ATV-Teams supports two runtime modes with different security profiles. Reachability is configured separately with `bind`.
...
When migrating from `local_trusted` to `authenticated`, ATV-Teams emits a one-time claim URL at startup:
