...
ATV-Teams encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

ATV-Teams protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. ATV-Teams does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, ATV-Teams can no longer guarantee
...
At runtime, ATV-Teams resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before ATV-Teams injects its
...
ATV-Teams best-effort enforces `0600` permissions when it creates or loads the
...
Provider-owned secrets can be linked without copying values into ATV-Teams by
using `managedMode: "external_reference"` plus a provider `externalRef`.
ATV-Teams stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted ATV-Teams Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in ATV-Teams company secrets**. Allowed credential sources are
workload identity attached to the ATV-Teams server (instance profile, IRSA, ECS
...
ATV-Teams `external_reference` secrets. This is a metadata-only link: ATV-Teams
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the ATV-Teams name and key, then
   submit.
...
cursors. Do not expect ATV-Teams to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the ATV-Teams runtime
...
- Keep managed create/rotate/delete permissions scoped to the ATV-Teams
...
dedicated ATV-Teams runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the ATV-Teams
  ...
  key bytes.
- `aws_secrets_manager`: back up ATV-Teams's database for vault metadata
  ...
  configured prefix; restore by pointing the same ATV-Teams company at the
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in ATV-Teams. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from ATV-Teams
...
For ATV-Teams Cloud, provision the server runtime IAM role/workload identity,
...
ATV-Teams secrets. Bootstrap material belongs in infrastructure IAM/workload
...
Use the CLI command for normal operations because it goes through the ATV-Teams
...
the value custodian; ATV-Teams stores metadata and provider version references,
