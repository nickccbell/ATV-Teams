...
ATV-Teams is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── paperclip/               # Core ATV-Teams skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with ATV-Teams env vars and a prompt
4. **Agent work** — The agent calls ATV-Teams's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between ATV-Teams and agent runtimes. Each adapter is a package with three modules:
...
- **Control plane, not execution plane** — ATV-Teams orchestrates agents; it doesn't run them
