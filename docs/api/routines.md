...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-ATV-Teams-Signature` + `X-ATV-Teams-Timestamp` header pair matching the trigger's signing mode.
