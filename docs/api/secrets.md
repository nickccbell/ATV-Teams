...
To link a provider-owned secret without copying the value into ATV-Teams, create
...
ATV-Teams stores the provider reference and a non-sensitive fingerprint only.
...
credentials must not be stored in ATV-Teams `company_secrets`.
...
Remote import links existing AWS Secrets Manager entries into ATV-Teams as
`external_reference` secrets. Import stores provider reference metadata only; it
does not copy the remote secret plaintext into ATV-Teams.
...
- `duplicate`: a ATV-Teams secret already links the same canonical provider
...
`provider_guardrail`. AWS refs under ATV-Teams's own managed namespace are
blocked as external references; use the ATV-Teams-managed secret flow for those
resources instead.
...
ATV-Teams `name`, `key`, optional ATV-Teams `description`,
`providerVersionRef`, and sanitized `providerMetadata`. Blank descriptions are
stored as `null`; AWS provider descriptions are not copied into ATV-Teams
...
- `imported`: ATV-Teams created an active `external_reference` secret and one
...
real value into the agent process environment. ATV-Teams's custody guarantees
