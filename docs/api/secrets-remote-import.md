...
ATV-Teams `external_reference` secrets without copying plaintext into
ATV-Teams.
...
`ListSecrets` only and import stores a ATV-Teams external reference plus
...
remote metadata, or copies secret plaintext into ATV-Teams.
...
- `conflict`: the suggested ATV-Teams `name` or `key` is already in use.
...
ATV-Teams-managed AWS secret through a broad runtime role.
...
Each row may include an optional ATV-Teams `description` entered during review;
...
the ATV-Teams runtime role can list the AWS secret but lacks
