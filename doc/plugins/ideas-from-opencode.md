...
ATV-Teams V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a ATV-Teams-shaped design.

Assumption for this document: ATV-Teams is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class ATV-Teams objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into ATV-Teams.

The main conclusion is:

- ATV-Teams should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- ATV-Teams should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- ATV-Teams should use multiple extension classes instead of one generic plugin bag:
...
If ATV-Teams does this well, the examples you listed become straightforward:
...
Relevant ATV-Teams files reviewed for current extension seams:
...
It is too dangerous for ATV-Teams core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for ATV-Teams — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
ATV-Teams should absolutely do this.
...
ATV-Teams should copy this discipline.
...
ATV-Teams should adopt this style for plugin actions, automations, and UI schemas.
...
ATV-Teams can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for ATV-Teams too.

## What ATV-Teams Should Not Copy Directly
...
ATV-Teams is a control plane for an operator-managed instance with company objects.
...
ATV-Teams is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

ATV-Teams should not auto-load arbitrary code from a workspace repo like `.paperclip/plugins` or project directories.
...
For ATV-Teams, equivalent hooks into:
...
ATV-Teams should not allow plugins to silently replace:
...
For ATV-Teams it would be risky because it combines:
...
ATV-Teams should require an explicit operator install step.

## Why ATV-Teams Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | ATV-Teams |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted ATV-Teams instance |
...
That means ATV-Teams should borrow the good ideas from `opencode` but use a stricter architecture.

## ATV-Teams Already Has Useful Pre-Plugin Seams

ATV-Teams has several extension-like seams already:
...
This is good news.
ATV-Teams does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended ATV-Teams Plugin Model
...
ATV-Teams already has this pattern implicitly:
...
- `plugin` means integration code that talks to ATV-Teams through a typed plugin protocol and capability model
...
- invoke explicit ATV-Teams actions through the API
...
ATV-Teams should treat plugin installation as a global instance-level action.

Examples:

- install `@paperclip/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over ATV-Teams objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

ATV-Teams already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for ATV-Teams too.
...
A `@paperclipai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running ATV-Teams instance.

A `create-paperclip-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how ATV-Teams achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the ATV-Teams server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. ATV-Teams's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.
...
## A Concrete SDK Shape For ATV-Teams
...
export default definePlugin({
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit ATV-Teams entities
    });
...
Any ATV-Teams plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific ATV-Teams objects, those are plugin data, not enable/disable boundaries.
...
- reuse core tables when the data is clearly part of ATV-Teams itself
...
If a concept is becoming part of ATV-Teams's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of ATV-Teams itself
...
### 3. Scope plugin data to ATV-Teams objects before adding custom schemas

A lot of plugin data naturally hangs off existing ATV-Teams objects:
...
If ATV-Teams eventually needs extension-owned tables, I would only allow that for:
...
- Board opens a project page and reviews the diff for that project's workspace without leaving ATV-Teams.
- Board reviews the diff after a run without leaving ATV-Teams.
...
This plugin syncs ATV-Teams work with Linear. It is useful for:

- importing backlog from Linear
- linking ATV-Teams issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between ATV-Teams and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into ATV-Teams.
- ATV-Teams issue status changes push to Linear and Linear comments arrive back through webhooks.
...
This plugin syncs ATV-Teams issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to ATV-Teams issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with ATV-Teams task governance
...
- Plugin settings:
  ...
  - whether new ATV-Teams issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active ATV-Teams issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into ATV-Teams.
- GitHub webhooks update status/comment state in ATV-Teams.
- A PR is linked back to the ATV-Teams issue so the board can follow delivery status.
...
This plugin surfaces external metrics and dashboards inside ATV-Teams. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating ATV-Teams issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full ATV-Teams plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the ATV-Teams dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a ATV-Teams issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of ATV-Teams core; keep it as additive signal and issue creation
...
This plugin pulls Stripe revenue and subscription data into ATV-Teams. It is useful for:
...
- Failed payment spikes or churn events can generate ATV-Teams issues for follow-up.
...
- Stripe data should stay additive to ATV-Teams core
...
ATV-Teams should not implement "an OpenCode-style generic in-process hook system."
ATV-Teams should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In ATV-Teams
