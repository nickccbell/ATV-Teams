...
ATV-Teams server + PostgreSQL 17. The database is health-checked before the server starts.
...
PostgreSQL data persists in a named Docker volume (`pgdata`). ATV-Teams data persists in `paperclip-data`.
...
For authenticated deployments, set one canonical public URL and let ATV-Teams derive auth/callback defaults:
...
- Adapter environment checks in ATV-Teams will surface missing auth/CLI prerequisites.

## Podman Quadlet (systemd)

The `docker/quadlet/` directory contains unit files to run ATV-Teams + PostgreSQL as systemd services via Podman Quadlet.

| File | Purpose |
|------|---------|
| `docker/quadlet/paperclip.pod` | Pod definition — groups containers into a shared network namespace |
| `docker/quadlet/paperclip.container` | ATV-Teams server — joins the pod, connects to Postgres at `127.0.0.1` |
...
- Containers in a pod share `localhost`, so ATV-Teams reaches Postgres at `127.0.0.1:5432`.
- PostgreSQL data persists in the `paperclip-pgdata` named volume.
- ATV-Teams data persists at `~/.local/share/paperclip`.
...
- Smoke script defaults host port to `3131` to avoid conflicts with local ATV-Teams on `3100`.
...
- ATV-Teams data persists via Docker volumes/bind mounts (compose) or at `~/.local/share/paperclip` (quadlet).
