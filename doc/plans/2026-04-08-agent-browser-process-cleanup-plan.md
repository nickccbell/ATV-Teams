...
Yes, there is a likely root cause in ATV-Teams's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, ATV-Teams can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: ATV-Teams already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- ATV-Teams may think it cancelled the run because the parent process is gone
...
This work is successful when ATV-Teams can:
...
- require every skill author to add bespoke browser teardown logic before ATV-Teams can clean up correctly
...
- make the leak measurable from ATV-Teams's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by ATV-Teams
...
Treat this as a ATV-Teams executor ownership bug, not an `agent-browser` bug.
