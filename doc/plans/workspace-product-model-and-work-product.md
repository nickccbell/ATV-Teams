...
ATV-Teams needs to support two very different but equally valid ways of working:
...
Today, ATV-Teams already has the beginnings of this model:
...
- making ATV-Teams feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing ATV-Teams users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted ATV-Teams deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning ATV-Teams into a full code review product
...
This object must be recorded explicitly so that ATV-Teams can:
...
ATV-Teams should treat PRs as a type of work product linked back to:
...
ATV-Teams already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted ATV-Teams must be a first-class deployment mode

ATV-Teams cannot assume that it is running on the same machine as the code.

In cloud deployments, ATV-Teams may:
...
- `Runtime service`: a process or service ATV-Teams owns or tracks for a workspace
...
It also lets cloud-hosted ATV-Teams deployments point at codebases and remotes without pretending the ATV-Teams host has direct filesystem access.
...
Without an explicit `execution workspace` record, ATV-Teams has nowhere stable to attach:
...
ATV-Teams needs a single place to show:
...
- ATV-Teams-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without ATV-Teams owning the execution host
...
## 1. ATV-Teams host and execution host must be decoupled
...
without the ATV-Teams host itself running local git or local preview processes.
...
ATV-Teams should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for ATV-Teams.
...
ATV-Teams should keep workspace automation centered on a small set of hooks:
...
ATV-Teams should provide the equivalent visibility in the project `Code` surface:
...
ATV-Teams should treat execution workspaces as the natural home for:
...
ATV-Teams should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
ATV-Teams should continue treating cleanup policy as part of the core workspace model:
...
ATV-Teams should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the ATV-Teams host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

ATV-Teams should borrow Worktrunk's good execution patterns while keeping the broader ATV-Teams model:
...
This is how ATV-Teams supports:
...
That keeps ATV-Teams focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted ATV-Teams deployments remain first-class

That is a better fit for ATV-Teams than either extreme:
