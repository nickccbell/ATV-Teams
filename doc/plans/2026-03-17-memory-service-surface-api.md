# ATV-Teams Memory Service Plan

## Goal

Define a ATV-Teams memory service and surface API that can sit above multiple memory backends, while preserving ATV-Teams's control-plane requirements:

- company scoping
- auditability
- provenance back to ATV-Teams work objects
- budget and cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md`, the AWS AgentCore comparison captured in [PAP-1274](/PAP/issues/PAP-1274), and the current ATV-Teams architecture in:
...
ATV-Teams should add a company-scoped memory control plane with company default plus agent override resolution, shared hook delivery, and full operation attribution, while leaving extraction and storage semantics to built-ins and plugins.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing ATV-Teams plugin runtime.

That keeps the core small and matches the broader ATV-Teams direction that specialized knowledge systems live at the edges.

### 4. ATV-Teams owns routing, provenance, and policy

Providers should not decide how ATV-Teams entities map to governance.

ATV-Teams core should own:
...
### 5. ATV-Teams exposes shared hooks, providers own extraction

ATV-Teams should emit a common set of memory hooks that built-ins, third-party adapters, and plugins can all use.

Those hooks should pass structured ATV-Teams source objects plus normalized metadata. The provider then decides how to extract from those objects.

ATV-Teams should not force one extraction pipeline or one canonical "memory text" transform before the provider sees the input.
...
ATV-Teams should still log those changes in activity and preserve full auditability. Approval-gated memory governance can remain an enterprise or future policy layer.
...
A mapping from a ATV-Teams target to a binding.
...
The normalized ATV-Teams scope passed into a provider request.
...
A normalized trigger emitted by ATV-Teams when something memory-relevant happens.
...
A normalized capture, record-write, query, browse, get, correction, or delete action performed through ATV-Teams.

ATV-Teams should log every memory operation whether the provider is local, plugin-backed, or external.

## Required Adapter Contract

The required core should be small enough to fit `memsearch`, `mem0`, `Memori`, `MemOS`, or `OpenViking`, but strong enough to satisfy ATV-Teams's attribution and inspectability requirements.
...
This contract intentionally does not force a provider to expose its internal graph, file tree, or ontology. It does require enough structure for ATV-Teams to browse, attribute, and audit what happened.
...
That lines up with the ATV-Teams plan at a high level: provider configuration, scoped writes, scoped retrieval, provider-managed extraction as a capability, and a browse and inspect surface.

The concrete changes ATV-Teams should take from AWS are:
...
AWS exposes list and retrieve surfaces directly. ATV-Teams should not make browse optional at the portable layer.
...
ATV-Teams should add cursor-based pagination to:
...
ATV-Teams should keep its own control-plane-centric model, but the adapter contract needs obvious places to map those concepts:
...
AWS exposes extraction jobs explicitly. ATV-Teams should too.
...
### 7. Keep ATV-Teams provenance primary

ATV-Teams should continue to center:
...
## What ATV-Teams Should Persist

ATV-Teams should not mirror the full provider memory corpus into Postgres unless the provider is a ATV-Teams-managed local provider.

ATV-Teams core should persist:
...
ATV-Teams should expose one shared hook system for memory.
...
1. `pre_run_hydrate`
   Before an agent run starts, ATV-Teams may call `query(... intent = "agent_preamble")` using the active binding.

2. `post_run_capture`
   After a run finishes, ATV-Teams may call `capture(...)` with structured run output, excerpts, and provenance.

3. `issue_comment_capture`
   When enabled on the binding, ATV-Teams may call `capture(...)` for selected issue comments.

4. `issue_document_capture`
   When enabled on the binding, ATV-Teams may call `capture(...)` for selected issue documents.
...
ATV-Teams should give agents both automatic recall and explicit tools, with simple guidance:
...
ATV-Teams needs a first-class UI for memory, otherwise providers become black boxes.
...
ATV-Teams should treat memory accounting as two related but distinct concerns:
...
- if a memory operation runs inside a normal ATV-Teams agent heartbeat and the model usage is already counted on that run, do not create a duplicate `cost_event`
- instead, store the memory operation with `attributionMode = "included_in_run"` and link it to the related `heartbeatRunId`
- if a memory provider makes a direct metered model call outside the agent run accounting path, the provider must report usage and ATV-Teams should create a `cost_event`
...
ATV-Teams should record evaluation-oriented metrics where possible:
...
This is important because a memory system that "works" but silently burns budget or silently fails extraction is not acceptable in ATV-Teams.
...
- it matches ATV-Teams's local-first posture
- it is inspectable
...
- How much source payload should ATV-Teams snapshot inside `memory_operations` for debugging without duplicating large transcripts?
...
- ATV-Teams owns bindings, resolution, hooks, provenance, policy, and attribution.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives ATV-Teams a stable memory service without locking the product to one memory philosophy or one vendor, and it integrates the AWS lessons without importing AWS's model into core.
