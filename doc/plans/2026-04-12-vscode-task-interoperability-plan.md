...
ATV-Teams should not replace its workspace runtime service model with VS Code tasks.
...
- ATV-Teams owns long-running workspace services and their desired state
- ATV-Teams shows operators exactly which named thing they are starting or stopping
- ATV-Teams distinguishes long-running services from one-shot jobs
...
This is directionally correct for ATV-Teams because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and ATV-Teams does not yet leverage existing workspace metadata.

## Recommendation

Keep ATV-Teams runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in ATV-Teams.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to ATV-Teams service or job semantics should become runnable in ATV-Teams.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than ATV-Teams runtime services.
...
That creates a bad fit if ATV-Teams tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace ATV-Teams's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

ATV-Teams should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command ATV-Teams can resolve
...
- selected variable substitution when ATV-Teams can resolve it safely from workspace context
- mapping task metadata into ATV-Teams readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types ATV-Teams cannot execute directly
...
ATV-Teams needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by ATV-Teams metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by ATV-Teams
...
For a workspace with `cwd`, ATV-Teams should look for `.vscode/tasks.json`.
...
ATV-Teams should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - ATV-Teams metadata marks it as a service, or
...
ATV-Teams-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what ATV-Teams would import and why.
...
Goal:
run supported imported service commands through the current ATV-Teams supervisor.
...
Success condition:
imported service commands behave like native ATV-Teams services once adopted.
...
- removal of ATV-Teams's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting ATV-Teams to VS Code and making the model worse for non-VS-Code repos
...
ATV-Teams should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing ATV-Teams workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
