...
This document defines the next-stage plan for ATV-Teams company import/export.

The core shift is:

- move from a ATV-Teams-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside ATV-Teams.
...
ATV-Teams already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just ATV-Teams
...
7. an always-emitted `.paperclip.yaml` sidecar for high-fidelity ATV-Teams-specific details
...
11. adapter-aware skill sync surfaces so ATV-Teams can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point ATV-Teams at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a ATV-Teams sidecar.
...
ATV-Teams must not redefine `SKILL.md`.
...
- ATV-Teams-specific fidelity lives in `.paperclip.yaml`
- ATV-Teams may resolve and install `SKILL.md` packages, but it must not require a ATV-Teams-only skill format
...
### 5.4 Base Package Vs ATV-Teams Extension
...
- base package:
  ...
  - no ATV-Teams-only runtime fields by default
- ATV-Teams extension:
  - `.paperclip.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by ATV-Teams tools as a sidecar while the base package stays readable
...
ATV-Teams import/export should support these entity kinds:
...
- ATV-Teams can treat “this set of agents came from team package X” as the imported-team model
...
ATV-Teams should support source references in package metadata with:
...
ATV-Teams should define a new adapter surface area around skills:
...
Planned ATV-Teams behavior:

- if an adapter supports read, ATV-Teams should show current skills in the UI
- if an adapter supports write, ATV-Teams should let the user enable/disable imported skills
- if an adapter supports sync, ATV-Teams should compute desired vs actual state and offer reconcile actions
...
## 10. Storage Model Inside ATV-Teams
...
ATV-Teams should add managed package/provenance records so imports are not anonymous one-off copies.
...
1. Should imported skill packages be stored as managed package files in ATV-Teams storage, or only referenced at import time?
...
3. Should ATV-Teams support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps ATV-Teams aligned with:
