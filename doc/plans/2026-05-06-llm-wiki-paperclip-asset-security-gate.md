# LLM Wiki ATV-Teams Asset And Work-Product Security Gate

Status: accepted Phase 5 policy
Date: 2026-05-06
Owner: Security engineering
Scope: ATV-Teams-derived ingestion into the LLM Wiki before any asset or work-product content indexing ships

## Decision

Phase 5 remains **fail-closed** for ATV-Teams assets and work products.

- ATV-Teams-derived **text extraction is allowed only** for issue titles/descriptions, issue comments, and issue documents.
- ATV-Teams **assets/attachments** and **issue work products** are **metadata-only** in Phase 5.
...
These source kinds may contribute body text to ATV-Teams-derived source bundles:

| Source kind | Allowed body fields | Reason |
| --- | --- | --- |
| Issue | `title`, `description`, identifier/status metadata | First-party ATV-Teams text under company ACL |
| Comment | `body` | First-party ATV-Teams text under company ACL |
| Document | `body`, `title`, `key`, revision metadata | First-party ATV-Teams text under company ACL |
...
- metadata-only references built from allowlisted structured fields already stored in ATV-Teams
...
- Every asset MIME type is treated as opaque for ATV-Teams-derived indexing.
...
- non-default-space routing for ATV-Teams-derived asset/work-product references

## Security Rationale

This gate exists because the current host surfaces have different trust properties:

- issue/comment/document text is first-party ATV-Teams content already exposed through company-scoped issue/document APIs
