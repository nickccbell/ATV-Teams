...
This document defines the V1 design for a ATV-Teams adapter that integrates with
...
4. synthesized stdout events for ATV-Teams UI/CLI
...
3. **Agent callback auth to ATV-Teams** uses a bootstrap exchange flow (no long-lived ATV-Teams key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from ATV-Teams endpoints, not full SKILL.md prompt injection.
...
## ATV-Teams Callback + Auth Flow (V1)
...
Goal: avoid putting long-lived ATV-Teams credentials in prompt text.

Flow:

1. Before launch/follow-up, ATV-Teams mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. ATV-Teams validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all ATV-Teams API calls.
...
1. Prompt includes a compact instruction to fetch skills from ATV-Teams.
...
4. Route by Cursor `agentId` to active ATV-Teams run context.
...
| Skill delivery | local skill dir injection | authenticated fetch from ATV-Teams skill endpoints |
| ATV-Teams auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from ATV-Teams context.
3. Add richer PR metadata surfacing in ATV-Teams UI.
