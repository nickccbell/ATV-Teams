...
Agents need API keys to authenticate with ATV-Teams. The current approach
...
**Trust model:** The adapter process runs on the same machine as the ATV-Teams
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** ATV-Teams generates a token and passes it directly to the agent
...
**Approach:** Similar to `claude setup-token` -- the developer runs a ATV-Teams CLI
...
managed by the ATV-Teams server itself.
...
1. A company admin (user or agent) generates an **invite URL** from ATV-Teams.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The ATV-Teams SKILL.md (or a link to it)
   - What information ATV-Teams needs from the agent (e.g. webhook URL, adapter
...
5. ATV-Teams stores the pending registration.
...
7. On approval, ATV-Teams provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** ATV-Teams issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "requiredFields": {
      ...
      "adapterType": "How ATV-Teams should send you heartbeats",
...
1. Generate a ATV-Teams invite link for the company.
...
4. A ATV-Teams company member approves the new agent.
5. ATV-Teams begins sending heartbeats to the OpenClaw webhook endpoint.
