...
This document defines the product and UI plan for skill management in ATV-Teams.
...
- ATV-Teams company import/export can include skills as package content
...
3. The UI must always tell the truth about what ATV-Teams knows:
   - desired state in ATV-Teams
...
ATV-Teams should treat skills at two scopes:
...
- created locally inside ATV-Teams later
...
7. “Understand what ATV-Teams knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a ATV-Teams-specific package layout
...
This matters because Codex and similar adapters may already have local skills that ATV-Teams did not install.

These should be clearly marked:

- external
- not managed by ATV-Teams
...
- `desired_only`: ATV-Teams wants it, adapter does not show it yet
- `external`: adapter has it but ATV-Teams does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, ATV-Teams only tracks desired state
...
- ATV-Teams can still track desired skills
...
- ATV-Teams can see adapter skills
...
- `.paperclip.yaml` may add ATV-Teams-specific fidelity, but should not replace the base shortname association model
...
- `.paperclip.yaml` may add ATV-Teams-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
ATV-Teams should have a company-scoped skill model or managed package model representing:
...
ATV-Teams should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too ATV-Teams-specific.
...
That gives ATV-Teams one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
