...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into ATV-Teams.
...
- current ATV-Teams plugin and adapter surfaces in this repo
...
## What ATV-Teams already has

ATV-Teams already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to ATV-Teams

These ideas from `pi-mono` fit ATV-Teams with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

ATV-Teams should continue exposing run and transcript events to plugins, for example:
...
This matches ATV-Teams's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

ATV-Teams already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

ATV-Teams already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in ATV-Teams, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for ATV-Teams core:

### 1. Arbitrary mutation hooks on control-plane decisions

ATV-Teams should not let general plugins rewrite:
...
ATV-Teams should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

ATV-Teams is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to ATV-Teams's board-and-issues model.

## Recommended ATV-Teams direction
...
If the question is "what works for ATV-Teams too?":
