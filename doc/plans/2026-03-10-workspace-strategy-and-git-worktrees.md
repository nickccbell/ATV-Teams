...
`PAP-447` asks how ATV-Teams should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same ATV-Teams instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into ATV-Teams:

- some operators use ATV-Teams to manage ATV-Teams and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as ATV-Teams
- Claude and Codex expose different built-in affordances, so ATV-Teams should not overfit to one tool

## Core Product Decision

ATV-Teams should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by ATV-Teams
...
- ATV-Teams should not assume that every adapter can see or use a host filesystem path directly
...
If ATV-Teams makes worktrees agent-first, it will blur:
...
- ATV-Teams core resolves and records execution workspace state
...
That gives ATV-Teams room to support:
...
- ATV-Teams handles workspace runtime services under the hood using project/workspace policy
...
Once ATV-Teams is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So ATV-Teams should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that ATV-Teams cannot infer
...
- when issue moves to done, should ATV-Teams auto-commit?
- should it auto-push?
...
ATV-Teams already has the right foundation for a project-first model.
...
For server-spawned local adapters, ATV-Teams already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that ATV-Teams should mirror directly

Implication:

- for `codex_local`, ATV-Teams should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but ATV-Teams should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, ATV-Teams should separate:
...
ATV-Teams should be able to express intentions such as:
...
These are not specific to ATV-Teams. They are a common property of working in a dev workspace, whether local or remote.
...
Do not model this as a ATV-Teams-specific `devServer` flag.
...
ATV-Teams should distinguish between:
...
ATV-Teams should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, ATV-Teams should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to ATV-Teams
...
For runtime services specifically, ATV-Teams should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as ATV-Teams-only dev-server flags
...
- same embedded DB / ATV-Teams instance can be reused through env/config injection when appropriate
...
- ATV-Teams can decide whether to reuse or start a fresh service deterministically
...
3. Test against ATV-Teams-on-ATV-Teams workflows first.
...
7. Workspace runtime services are modeled generically, not as ATV-Teams-specific dev-server toggles.
