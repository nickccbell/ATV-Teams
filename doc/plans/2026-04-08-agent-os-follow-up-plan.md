...
Turn the `agent-os` research into a low-risk ATV-Teams execution plan that preserves ATV-Teams's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

ATV-Teams should not absorb `agent-os` as a product model or orchestration layer.

ATV-Teams should evaluate `agent-os` in three narrow areas:
...
This work is successful when ATV-Teams has:
...
- a proposed runtime capability model that fits current ATV-Teams adapters
...
- replace ATV-Teams heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to ATV-Teams core

## Existing ATV-Teams integration points
...
- prove that ATV-Teams can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep ATV-Teams's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a ATV-Teams-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some ATV-Teams workloads
...
- identify where ATV-Teams should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in ATV-Teams
...
- runtime experimentation could blur the boundary between ATV-Teams as control plane and ATV-Teams as execution platform
