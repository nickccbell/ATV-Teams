# ATV-Teams Specification

Target specification for the ATV-Teams control plane. Living document — updated incrementally during spec interviews.

---

## 1. Company Model [DRAFT]

A Company is a first-order object. One ATV-Teams instance runs multiple Companies. A Company does not have a standalone "goal" field — its direction is defined by its set of Initiatives (see Task Hierarchy Mapping).
...
Concepts like SOUL.md (identity/mission) and HEARTBEAT.md (loop definition) are **not part of the ATV-Teams protocol**. They are adapter-specific configurations. For example, an OpenClaw adapter might use SOUL.md and HEARTBEAT.md files. A Claude Code adapter might use CLAUDE.md. A bare Python script might use command-line args.

ATV-Teams doesn't prescribe how an agent defines its identity or behavior. It provides the control plane; the adapter defines the agent's inner workings.
...
#### ATV-Teams Protocol (What ATV-Teams Knows)

At the protocol level, ATV-Teams tracks:
...
- **Fat payload** — ATV-Teams bundles relevant context (current tasks, messages, company state, metrics) into the heartbeat invocation. Suited for simple/stateless agents that can't call back to ATV-Teams.
- **Thin ping** — Heartbeat is just a wake-up signal. Agent calls ATV-Teams's API to fetch whatever context it needs. Suited for sophisticated agents that manage their own state.

#### Minimum Contract

The minimum requirement to be a ATV-Teams agent: **be callable.** That's it. ATV-Teams can invoke you via command or webhook. No requirement to report back — ATV-Teams infers basic status from process liveness when it can.

#### Integration Levels

Beyond the minimum, ATV-Teams provides progressively richer integration:

1. **Callable** (minimum) — ATV-Teams can start you. That's the only contract.
...
ATV-Teams ships **default agents** that demonstrate full integration: progress tracking, cost instrumentation, and a **ATV-Teams skill** (a Claude Code skill for interacting with the ATV-Teams API) for task management. These serve as both useful defaults and reference implementations for adapter authors.
...
Agents can create tasks and assign them to agents outside their reporting line. This is the mechanism for cross-team collaboration. These rules are primarily encoded in the ATV-Teams SKILL.md which is recommended for all agents. ATV-Teams the app enforces the tooling and some light governance, but the cross-team rules below are mainly implemented by agent decisions.
...
The heartbeat is a protocol, not a runtime. ATV-Teams defines how to initiate an agent's cycle. What the agent does with that cycle — how long it runs, whether it's task-scoped or continuous — is entirely up to the agent.
...
This is the full adapter contract. `invoke` starts the agent, `status` lets ATV-Teams check on it, `cancel` enables the board's pause functionality. Everything else (cost reporting, task updates) is optional and flows through the ATV-Teams REST API.

### What ATV-Teams Controls
...
### What ATV-Teams Does NOT Control
...
- Health monitoring — how does ATV-Teams distinguish "stuck" from "working on a long task"?
...
Token/LLM cost budgeting is a core part of ATV-Teams. External revenue and expense tracking is a future plugin.

### Cost Reporting

Fully-instrumented Agents report token/API usage back to ATV-Teams. Costs are tracked at every level:
...
### ATV-Teams Skill (SKILL.md)

A skill definition that teaches agents how to interact with ATV-Teams. Provides:
...
The key constraint: it must be trivial to go from "I'm trying this on my machine" to "my agents are running on remote servers talking to my ATV-Teams instance."

#### Agent Authentication

When a user creates an Agent, ATV-Teams generates a **connection string** containing: the server URL, an API key, and instructions for how to authenticate. The Agent is assumed to be capable of figuring out how to call the API with its token/key from there.

Flow:

1. Human creates an Agent in the UI
2. ATV-Teams generates a connection string (URL + key + instructions)
...
When a human completes a task, if the requesting agent's adapter supports **pingbacks** (e.g. OpenClaw hooks), ATV-Teams sends a notification to wake that agent. This keeps humans rare but possible participants in the workflow.

The agents are discouraged from assigning tasks to humans in the ATV-Teams SKILL, but sometimes it's unavoidable.
...
ATV-Teams manages task-linked work artifacts: issue documents (rich-text plans, specs, notes attached to issues) and file attachments. Agents read and write these through the API as part of normal task execution. Full delivery infrastructure (code repos, deployments, production runtime) remains the agent's domain — ATV-Teams orchestrates the work, not the build pipeline.
...
When an agent crashes or disappears mid-task, ATV-Teams does **not** auto-reassign or auto-release the task. Instead:

- ATV-Teams surfaces stale tasks (tasks in `in_progress` with no recent activity) through dashboards and reporting
- ATV-Teams does not fail silently — the auditing and visibility tools make problems obvious
...
**Principle: ATV-Teams reports problems, it doesn't silently fix them.** Automatic recovery hides failures. Good visibility lets the right entity (human or agent) decide what to do.

### Plugin / Extension Architecture

The core ATV-Teams system must be extensible. Features like knowledge bases, external revenue tracking, and new Agent Adapters should be addable as **plugins** without modifying core. This means:
...
**Full loop with one adapter.** V1 must demonstrate the complete ATV-Teams cycle end-to-end, even if narrow.
...
**Anti-goal for core.** The knowledge base is not part of the ATV-Teams core — it will be a plugin. The task system + comments + agent descriptions provide sufficient shared context.
...
Things ATV-Teams explicitly does **not** do:

- **Not an Agent runtime** — ATV-Teams orchestrates, Agents run elsewhere
...
1. **Unopinionated about how you run your Agents.** Any language, any framework, any runtime. ATV-Teams is the control plane, not the execution plane.
