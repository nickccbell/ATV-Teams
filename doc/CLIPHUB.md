...
ClipHub is the public registry where people share, discover, and download ATV-Teams company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to ATV-Teams what a package registry is to a programming language. ATV-Teams already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in ATV-Teams — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own ATV-Teams instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the ATV-Teams spec. It contains:
...
```
Downloads the template and creates a new company in your local ATV-Teams instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within ATV-Teams, export your company as a template, then publish:
...
ClipHub is a **separate service** from ATV-Teams itself. ATV-Teams is self-hosted; ClipHub is a hosted registry that ATV-Teams instances talk to.
...
| **ATV-Teams CLI** | `paperclipai install`, `paperclipai publish`, `paperclipai cliphub sync` — built into ATV-Teams |
| **ATV-Teams UI** | "Browse ClipHub" panel in the ATV-Teams web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with ATV-Teams) |
| Backend | TypeScript + Hono (consistent with ATV-Teams) |
| Database | PostgreSQL |
...
5. ATV-Teams creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in ATV-Teams until it works well
...
## Relationship to ATV-Teams

ClipHub is **not required** to use ATV-Teams. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to ATV-Teams what a package registry is to a language runtime: optional, but transformative.
...
- [ ] "Browse ClipHub" panel in ATV-Teams web UI
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with ATV-Teams's own philosophy)
