...
1. Start ATV-Teams in auth mode.
```bash
...
3. In ATV-Teams UI, go to `http://127.0.0.1:3100/CLA/company/settings`.
...
5. Approve the join request in ATV-Teams UI, then confirm the OpenClaw agent appears in CLA agents.
...
- This is a separate approval from ATV-Teams invite approval. You must approve the pending device in OpenClaw itself.
...
- Ask it to create a new CLA issue in ATV-Teams with unique title `OPENCLAW_CASE_C_CREATED_<timestamp>`.
- Verify in ATV-Teams UI that new issue exists.
