...
ATV-Teams supports two runtime modes:
...
ATV-Teams now treats **bind** as a separate concern from auth:
...
When running `authenticated` mode, if the only instance admin is `local-board`, ATV-Teams emits a startup warning with a one-time high-entropy claim URL.
