...
This spec defines how ATV-Teams actually runs agents while staying runtime-agnostic.
...
1. ATV-Teams is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, ATV-Teams manages lifecycle, and on exit ATV-Teams parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `paperclip.skill` (shared ATV-Teams skill text block)
