...
For `claude_local` and `codex_local`, ATV-Teams assumes the CLI is already installed and authenticated on the host machine.
...
ATV-Teams stores resumable session state per `(agent, taskKey, adapterType)`.
