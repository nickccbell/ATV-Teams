...
describe("createZipArchive", () => {
  it("writes a zip archive with the export root path prefixed into each entry", () => {
    ...
    expect(readString(archive, 30, firstNameLength)).toBe("atv-demo/agents/ceo/AGENTS.md");
    ...
    expect(readString(archive, secondOffset + 30, secondNameLength)).toBe("atv-demo/COMPANY.md");
  ...
  it("reads a ATV-Teams zip archive back into rootPath and file contents", async () => {
    const archive = createZipArchive(
      {
        "COMPANY.md": "# Company\n",
        "agents/ceo/AGENTS.md": "# CEO\n",
  ...
  it("reads standard DEFLATE zip archives created outside ATV-Teams", async () => {
  ...
  it("ignores directory entries from standard zip archives", async () => {
    const archive = createZipArchiveWithDirectoryEntries("atv-demo");
