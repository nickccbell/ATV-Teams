...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("atv-teams-worktree-enabled") !== "true") return null;

  const name = readMetaContent("atv-teams-worktree-name");
  const color = normalizeHexColor(readMetaContent("atv-teams-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("atv-teams-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
