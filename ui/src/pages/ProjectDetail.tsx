...
function ProjectIssuesList({ projectId, companyId }: { projectId: string; companyId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey="atv-teams:project-issues-view"
...
function ProjectPluginOperationsList({
  ...
  return (
    <IssuesList
      ...
      liveIssueIds={liveIssueIds}
      projectId={projectId}
      viewStateKey={`atv-teams:project-plugin-operations-view:${pluginKey}`}
      onUpdateIssue={(id, data) => updateIssue.mutate({ id, data })}
    />
  );
}
...
export function ProjectDetail() {
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (project?.id) {
      try { cachedTab = localStorage.getItem(`atv-teams:project-tab:${project.id}`); } catch {}
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (project?.id) {
      try { localStorage.setItem(`atv-teams:project-tab:${project.id}`, tab); } catch {}
    }
