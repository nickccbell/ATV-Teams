...
describe("BreadcrumbContext", () => {
  ...
  it("builds page titles with the selected company name before ATV-Teams", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "Anachronist Wiki")).toBe(
      "Inbox • Anachronist Wiki • ATV-Teams",
    );
    expect(
      buildDocumentTitle(
    ...
    ).toBe("PAP-3515 • Issues • Anachronist Wiki • ATV-Teams");
  });

  it("omits blank company names from page titles", () => {
    expect(buildDocumentTitle([{ label: "Inbox" }], "  ")).toBe("Inbox • ATV-Teams");
    expect(buildDocumentTitle([], null)).toBe("ATV-Teams");
  });
});
