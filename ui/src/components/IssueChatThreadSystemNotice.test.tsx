...
describe("IssueChatThread system notice routing", () => {
  it("renders authorType=system comments as a SystemNotice rather than a user bubble", () => {
    ...
    expect(container.textContent).toContain("ATV-Teams needs a disposition");
  ...
  it("labels system notice source as ATV-Teams when no run agent can be resolved", () => {
    const comment: IssueChatComment = {
    ...
    const status = container.querySelector('[role="status"]');
    expect(status).not.toBeNull();
    expect(status?.textContent).toContain("ATV-Teams");
    expect(status?.textContent).not.toContain("You");
  });

  it("falls back to ATV-Teams in the system notice header when run agent is unknown to agentMap", () => {
    const comment: IssueChatComment = {
    ...
    expect(sourceLink?.textContent).toBe("ATV-Teams");
  });
