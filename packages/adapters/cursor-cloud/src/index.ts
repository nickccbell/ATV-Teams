...
Use when:
- You want ATV-Teams to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across ATV-Teams heartbeats
- You want ATV-Teams to keep task state while Cursor handles remote code execution
...
Notes:
- ATV-Teams reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each ATV-Teams heartbeat maps to a Cursor run on that durable agent.
- ATV-Teams injects PAPERCLIP_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- ATV-Teams remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
