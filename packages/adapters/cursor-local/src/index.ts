...
Use when:
- You want ATV-Teams to run Cursor Agent CLI locally as the agent runtime
...
- ATV-Teams auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$paperclip" and related skills on local runs.
- ATV-Teams auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
