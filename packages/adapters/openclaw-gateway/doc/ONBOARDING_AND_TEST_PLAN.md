...
## Scope
This plan is now **gateway-only**. ATV-Teams supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses ATV-Teams skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a ATV-Teams task.
