...
Use when:
- You want ATV-Teams to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the ATV-Teams server.
...
- paperclipApiUrl (string, optional): absolute ATV-Teams base URL advertised in wake text
...
- paperclip (object): standardized ATV-Teams context added to every gateway agent request
- paperclip.workspace (object, optional): resolved execution workspace for this run
- paperclip.workspaces (array, optional): additional workspace hints ATV-Teams exposed to the run
