...
Use when:
- You want ATV-Teams to run OpenCode locally as the agent runtime
...
- dangerouslySkipPermissions (boolean, optional): inject a runtime OpenCode config that allows \`external_directory\` access without interactive prompts; defaults to true for unattended ATV-Teams runs
...
- ATV-Teams requires an explicit \`model\` value for \`opencode_local\` agents.
...
- When \`dangerouslySkipPermissions\` is enabled, ATV-Teams injects a temporary \
