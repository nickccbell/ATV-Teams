...
| [`paperclip-distill`](./paperclip-distill/SKILL.md) | Cursor-window, distill, or backfill operation on ATV-Teams activity — write a wiki-insightful project page, decisions log, and history note. |
...
- Each skill names the input it expects (e.g. an operation issue with `originKind` ending in `:ingest`, a captured `raw/` path, a ATV-Teams source bundle).
