...
description: Use when an operation issue is a ATV-Teams cursor-window, distill, or backfill — `operationType: "distill"` or `"backfill"` and the body references a ATV-Teams source bundle for a project or root issue. Turn raw ATV-Teams activity into a wiki-insightful project page, decisions log, and history note. This skill exists specifically to replace the stiff, datestamp-heavy templated output that the deterministic distiller produces.
---

# ATV-Teams Distill

Distill ATV-Teams project, issue, comment, and document activity into durable wiki pages. The success criterion is **wiki-insightful, not procedural**: a reader who has never seen ATV-Teams should learn what the project is, what was decided, what is at risk, and what the current state is — without scanning a list of `## [YYYY-MM-DD]` headers.

## When this skill is needed

- Cursor-window distillation: the routine fed you a bounded source bundle of recent ATV-Teams activity for one project or root issue.
...
In Phase 1, every ATV-Teams distill, backfill, and cursor-window operation writes into the
...
write ATV-Teams-derived pages into a non-default space.

This rule is destination-only. The ATV-Teams source scope (which projects, root issues,
comments, documents are read) is set elsewhere in the operation issue and is independent of
the destination.

## Inputs

- A ATV-Teams source bundle (issue list, comment refs, document refs, source hash, cursor window).
...
## ATV-Teams Asset Gate

Do not treat ATV-Teams assets/attachments or issue work products as source text for this skill.

- Allowed ATV-Teams body text: issue descriptions, comment bodies, document bodies.
...
2. **Procedural status lists.** `Issue mix: 3 todo, 5 in_progress, 2 done` tells the reader nothing they could not read off ATV-Teams directly. State *what is happening and why it matters*, then cite the issues that constitute the evidence.
...
4. **Write `wiki/projects/<slug>/standup.md` first.** Every ATV-Teams project represented in the wiki must have this file. It is the executive standup: where the project stands today, what changed recently, what is blocked or risky, and what happens next. Use stable sections, in this order:
   ...
   - **Next Actions** — concrete next actions and owners inferred from ATV-Teams issues, not vague aspirations.
   - **Links** — durable wiki project page and relevant ATV-Teams project/issues/documents.
   Rewrite the standup to today's state. Do not append endless dated sections; the audit trail belongs in `wiki/log.md` and ATV-Teams comments.
5. **Write `wiki/projects/<slug>/index.md`** with these stable sections, in this order:
   ...
   - **References** — readable links to the current standup and supporting ATV-Teams tasks/documents. Keep hashes and cursor ids out of the narrative.
...
- Cite ATV-Teams source refs inline using their issue identifier (e.g. `PAP-3179`), not opaque UUIDs.
...
- [ ] The project page reads as wiki content, not as a ATV-Teams status report. A reader new to the company should understand what the project is.
...
`wiki_search`, `wiki_read_page`, `wiki_write_page`, `wiki_list_sources`, `wiki_read_source`. Always include the operation issue's `wikiId` and `spaceSlug`. The ATV-Teams source bundle arrives as part of the operation context — you do not need to assemble it.
