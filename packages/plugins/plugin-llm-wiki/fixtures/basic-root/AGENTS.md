...
└── wiki/             # generated, owned by you
    ...
    ├── projects/     # ATV-Teams project overviews, standups, decisions, and history
    │   └── <slug>/
    │       ├── index.md
    │       ├── standup.md
    │       ├── decisions.md
...
ATV-Teams project material lives only under `wiki/projects/<project-slug>/`. Do not create a top-level `projects/` directory.
...
- Update `standup.md` whenever ATV-Teams project, issue, plan, comment, blocker, approval, or status history materially changes the project's current state. Do not append endless dated sections; rewrite it as today's concise status.
- Project writing should be editorial and concept-grouped. Do not dump issue queues, UUIDs, raw metadata, or date-heavy ledgers into project pages. Reference ATV-Teams tasks with human issue links where useful, but make headings and paragraphs explain the concepts, decisions, completed work, next work, and blockers in plain executive language.
...
Triggered when ATV-Teams project, issue, plan, comment, blocker, or status history is distilled into the wiki.

1. Create or update `wiki/projects/<project-slug>/standup.md` first. Every ATV-Teams project represented in the wiki must have one. Keep stable sections for executive readout, what changed, decisions, blockers/risks, next actions, and links.
...
5. Always cite ATV-Teams source material with readable links to issue identifiers, document keys, issue documents, approvals, and raw/source pages. Do not put UUIDs in prose unless the UUID itself is the subject.
