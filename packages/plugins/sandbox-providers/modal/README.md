...
Modal's official JS SDK README pins support to **Node 22 or later**. ATV-Teams's repo baseline is currently `node >= 20`; empirically `modal@0.7.4` imports and operates against the Modal API under Node 20, so the plugin runs there today, but the vendor support contract is Node 22+. The plugin logs a startup warning when it detects Node `< 22`. Operators who can pin their ATV-Teams runtime to Node 22+ should do so; treat Node-20 usage as best-effort until the host bumps its baseline.
...
| `tokenId` / `tokenSecret` | yes | Modal auth tokens. Both must be provided together. ATV-Teams stores pasted values as company secrets. The plugin worker runs in a child process that does not inherit host env vars, so `MODAL_TOKEN_ID` / `MODAL_TOKEN_SECRET` set on the ATV-Teams server are **not** read by the plugin — provide the tokens in this form. |
...
2. Install the plugin from the ATV-Teams Plugins page.
...
5. Run at least one ATV-Teams task with a remote-managed adapter (for example `claude_local`) bound to that environment. The adapter should provision the sandbox, run commands in it, and clean it up.
