...
export interface AdapterEnvironmentTestContext {
   ...
   *
   * If omitted (or `kind === "local"`), the adapter tests on the ATV-Teams
...
export interface AdapterRuntimeCommandSpec {
  /**
   * The command ATV-Teams should execute for this adapter in the current config.
   */
  command: string;
  /**
