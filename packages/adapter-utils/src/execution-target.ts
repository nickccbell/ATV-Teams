...
function resolveDefaultPaperclipApiUrl(): string {
  ...
  // 3100 matches the default ATV-Teams dev server port when the runtime does not provide one.
 ...
 * the directory state inside the environment, not on the ATV-Teams host.
 *
...
export async function startAdapterExecutionTargetPaperclipBridge(input: {
  ...
  if (hostApiToken.length === 0) {
    throw new Error("Sandbox bridge mode requires a host-side ATV-Teams API token.");
  }
