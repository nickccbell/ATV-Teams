...
describe("codex local adapter skill injection", () => {
  ...
  it("repairs a Codex ATV-Teams skill symlink that still points at another live checkout", async () => {
  ...
  it("preserves a custom Codex skill symlink outside ATV-Teams repo checkouts", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
  ...
  it("prunes broken symlinks for unavailable ATV-Teams repo skills before Codex starts", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
    const oldRepo = await makeTempDir("paperclip-codex-old-");
    const skillsHome = await makeTempDir("paperclip-codex-home-");
    cleanupDirs.add(currentRepo);
  ...
  it("preserves other live ATV-Teams skill symlinks in the shared workspace skill directory", async () => {
    const currentRepo = await makeTempDir("paperclip-codex-current-");
    const skillsHome = await makeTempDir("paperclip-codex-home-");
    cleanupDirs.add(currentRepo);
    cleanupDirs.add(skillsHome);
