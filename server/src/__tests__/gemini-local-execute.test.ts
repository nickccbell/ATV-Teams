...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects paperclip env vars", async () => {
    ...
    try {
      ...
      expect(promptArg).toContain("ATV-Teams runtime note:");
      ...
      expect(invocationPrompt).toContain("ATV-Teams runtime note:");
      expect(invocationPrompt).toContain("PAPERCLIP_API_URL");
      expect(invocationPrompt).toContain("ATV-Teams API access note:");
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    ...
    try {
      ...
      expect(promptArg).toContain("## ATV-Teams Resume Delta");
