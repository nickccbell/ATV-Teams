...
function unavailableProvider(
  ...
  return {
    ...
    async createSecret() {
      throw unprocessable(`${id} provider is not configured for ATV-Teams-managed values`);
    },
    async createVersion() {
      throw unprocessable(`${id} provider is not configured for ATV-Teams-managed values`);
    },
    ...
    async deleteOrArchive() {
      // External references are metadata-only in ATV-Teams for unconfigured providers.
    },
    async healthCheck() {
      return {
        provider: id,
