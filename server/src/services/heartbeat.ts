...
export function heartbeatService(db: Db, options: HeartbeatServiceOptions = {}) {
  ...
  function monitorRecoveryComment(input: {
    ...
    return [
      `ATV-Teams cleared the scheduled external-service monitor for ${label} because ${reason}.`,
      "",
  ...
  async function cancelQueuedRunForBlockedDependencies(
    ...
    const now = new Date();
    const reason =
      "Cancelled because issue dependencies are still blocked; ATV-Teams will wake the assignee when blockers resolve";
  ...
  function buildImmediateExecutionPathRecoveryComment(input: {
    ...
    if (input.status === "todo") {
      return (
        "ATV-Teams automatically retried dispatch for this assigned `todo` issue during terminal run recovery, " +
    ...
    return (
      "ATV-Teams automatically retried continuation for this assigned `in_progress` issue during terminal run " +
