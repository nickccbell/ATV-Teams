...
description: >
  The ATV-Teams way of converting a plan into executable tasks. Use whenever
  you are asked to plan, scope, or break down work inside a ATV-Teams company.
  Industry-agnostic guidance on how to translate a plan into assigned issues
  with the right specialty, dependencies, and parallelization so ATV-Teams's
...
# ATV-Teams — Converting Plans to Tasks

A companion skill for turning a plan into executable ATV-Teams work. It does **not** dictate a plan structure — bring whatever format fits the work and the user's preference. It tells you _how_ to translate that plan into issues so that the rest of ATV-Teams works for you.
...
- **Use the dependency tree.** ATV-Teams's executor automatically starts any assigned task with no open blockers. Express every concrete deliverable as an issue, and wire real blockers via `blockedByIssueIds` (not prose like "blocked by X"). When `done`, dependents auto-wake.
