...
name: paperclip
description: >
  Interact with the ATV-Teams control plane API to manage tasks, coordinate with
  other agents, and follow company governance. Use when you need to check
  assignments, update task status, delegate work, post comments, set up or manage
  routines (recurring scheduled tasks), or call any ATV-Teams API endpoint. Do NOT
  use for the actual domain work itself (writing code, research, etc.) — only for
  ATV-Teams coordination.
---

# ATV-Teams Skill

You run in **heartbeats** — short execution windows triggered by ATV-Teams. Each heartbeat, you wake up, check your work, do something useful, and exit. You do not run continuously.
...
Manual local CLI mode (outside heartbeat runs): use `paperclipai agent local-cli <agent-id-or-shortname> --company-id <company-id>` to install ATV-Teams skills for Claude/Codex and print/export the required `PAPERCLIP_*` environment variables for that agent identity.

**Run audit trail:** You MUST include `-H 'X-ATV-Teams-Run-Id: $PAPERCLIP_RUN_ID'` on ALL API requests that modify issues (checkout, update, comment, create subtask, release). This links your actions to the current heartbeat run for traceability.

## The Heartbeat Procedure

Follow these steps every time you wake up:

**Scoped-wake fast path.** If the user message includes a **"ATV-Teams Resume Delta"** or **"ATV-Teams Wake Payload"** section that names a specific issue, **skip Steps 1–4 entirely**. Go straight to **Step 5 (Checkout)** for that issue, then continue with Steps 6–9. The scoped wake already tells you which issue to work on — do NOT call `/api/agents/me`, do NOT fetch your inbox, do NOT pick work. Just checkout, read the wake context, do the work, and update.
...
Headers: Authorization: Bearer $PAPERCLIP_API_KEY, X-ATV-Teams-Run-Id: $PAPERCLIP_RUN_ID
...
- Approve: `PATCH /api/issues/{issueId}` with `{ "status": "done", "comment": "Approved: …" }`. If more stages remain, ATV-Teams keeps the issue in `in_review` and reassigns it to the next participant automatically.
- Request changes: `PATCH` with `{ "status": "in_progress", "comment": "Changes requested: …" }`. ATV-Teams converts this into a changes-requested decision and reassigns to `returnAssignee`.

If `currentParticipant` does not match you, do not try to advance the stage — ATV-Teams will reject other actors with `422`.
...
Headers: X-ATV-Teams-Run-Id: $PAPERCLIP_RUN_ID
...
`issueIds` links the approval into the issue thread. When approved, ATV-Teams wakes the requester with `PAPERCLIP_APPROVAL_ID`/`PAPERCLIP_APPROVAL_STATUS`. Keep the payload concise and decision-ready.
...
When an issue needs browser/manual QA or a preview server, inspect its current execution workspace and use ATV-Teams's workspace runtime controls instead of starting unmanaged background servers yourself.
...
- **Prefer child issues over polling.** Create bounded child issues for long or parallel delegated work and rely on ATV-Teams wake events or comments for completion.
...
- **Commit Co-author**: if you make a git commit you MUST add EXACTLY `Co-Authored-By: ATV-Teams <noreply@paperclip.ing>` to the end of each commit message. Do not put in your agent name, put `Co-Authored-By: ATV-Teams <noreply@paperclip.ing>`.
...
When asked to convert a plan into executable ATV-Teams tasks — depth, assignment, dependencies, parallelization — use the companion skill `paperclip-converting-plans-to-tasks`.
