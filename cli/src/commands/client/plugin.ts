...
export function renderLocalPluginInstallHint(packagePath: string): string {
  return [
    ...
    pc.dim(`Keep ${pc.cyan("pnpm dev")} running in ${packagePath}; ATV-Teams watches rebuilt dist output and reloads the plugin worker.`),
  ].join("\n");
}
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .description("Scaffold a local ATV-Teams plugin project")
