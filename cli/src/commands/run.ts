...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  p.log.step("Starting ATV-Teams server...");
...
function ensureDevWorkspaceBuildDeps(projectRoot: string): void {
  ...
  if (result.error) {
    throw new Error(
      `Failed to prepare workspace build artifacts before starting the ATV-Teams dev server.\n${formatError(result.error)}`,
    );
  }
...
async function importServerEntry(): Promise<StartedServer> {
  ...
  try {
    ...
    if (isModuleNotFoundError(err) && missingServerEntrypoint) {
      throw new Error(
        `Could not locate a ATV-Teams server entrypoint.\n` +
          `Tried: ${devEntry}, @paperclipai/server\n` +
          `${formatError(err)}`,
      );
    }
    throw new Error(
      `ATV-Teams server failed to start.\n` +
        `${formatError(err)}`,
    );
  }
}
...
async function startServerFromModule(mod: unknown, label: string): Promise<StartedServer> {
  ...
  if (typeof startServer !== "function") {
    throw new Error(`ATV-Teams server entrypoint did not export startServer(): ${label}`);
  }
