...
description: >
  ...
  is already running in ATV-Teams.
---
...
└── .paperclip.yaml            (ATV-Teams vendor extension)
```
...
- For companies generated from a repo, add a references footer at the bottom of COMPANY.md body:
  `Generated from [repo-name](repo-url) with the company-creator skill from [ATV-Teams](https://github.com/paperclipai/paperclip)`
...
- Citations and references: link to the source repo (if from-repo), link to the Agent Companies spec (https://agentcompanies.io/specification), and link to ATV-Teams (https://github.com/paperclipai/paperclip)
...
The `.paperclip.yaml` file is the ATV-Teams vendor extension. It configures adapters and env inputs per agent.

### Adapter Rules

**Do not specify an adapter unless the repo or user context warrants it.** If you don't know what adapter the user wants, omit the adapter block entirely — ATV-Teams will use its default. Specifying an unknown adapter type causes an import error.

ATV-Teams's supported adapter types (these are the ONLY valid values):
